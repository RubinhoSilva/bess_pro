...
export const CableSizingResult: React.FC<CableSizingResultProps> = ({ results }) => {
  const { cableSizingResults } = results;

  if (!cableSizingResults || cableSizingResults.length === 0) {
    return (
      <Card className="bg-white dark:bg-slate-800/50 border-gray-300 dark:border-slate-700 h-full">
        <CardHeader>
        ...
        <CardContent>
          <p className="text-slate-400 text-center py-8">
            Nenhum cálculo de circuito CA foi configurado ou salvo.
          </p>
        </CardContent>
      </Card>
    );
        ...
        {cableSizingResults.map((res, index) => (
          <div key={index} className="p-3 bg-slate-700/50 rounded-lg">
            <h4 className="font-semibold text-purple-300 flex items-center gap-2 mb-2">
              <Unplug className="w-4 h-4" /> {res.inverterName}
            </h4>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="flex items-center gap-2">
                <Zap className="w-4 h-4 text-yellow-400" /> 
                Corrente: <span className="font-mono">{res.correnteProjeto.toFixed(2)} A</span>
              </div>
              <div className="flex items-center gap-2">
                <Scale className="w-4 h-4 text-blue-400" /> 
                Cabo: <span className="font-mono">{res.secaoMinimaCalculada} mm²</span>
              </div>
