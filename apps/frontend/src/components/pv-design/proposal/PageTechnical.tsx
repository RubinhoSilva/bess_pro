...
const EquipmentCard: React.FC<EquipmentCardProps> = ({ title, icon, data }) => (
  <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
    <h4 className="font-bold text-lg mb-3 flex items-center gap-2 text-slate-100">
      {icon} {title}
    </h4>
...
export const PageTechnical: React.FC<PageTechnicalProps> = ({ results }) => {
  const { potenciaPico, numeroModulos, selectedModule, selectedInverters = [] } = results;
  
  // Cálculos
  const areaEstimada = numeroModulos * 2.7; // Estimativa padrão de 2.7m² por módulo
  const geracaoEstimadaAnual = results.geracaoEstimadaMensal.reduce((acc, val) => acc + val, 0);
  const numeroTotalInversores = selectedInverters.reduce((acc, inv) => acc + inv.quantity, 0);

  const moduleData: Record<string, string> = selectedModule ? {
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          ...
          <div className="mt-4 space-y-2 text-base">
            <DataRow 
              label="Potência de Pico do Sistema" 
              value={potenciaPico.toFixed(2)} 
              unit="kWp" 
            />
            <DataRow 
              label="Número de Módulos Fotovoltaicos" 
              value={numeroModulos} 
              unit="unidades" 
            />
            <DataRow 
              label="Número Total de Inversores" 
              value={numeroTotalInversores} 
              unit="unidades" 
            />
            <DataRow 
            ...
            />
            <DataRow 
              label="Geração Anual de Energia Estimada" 
        ...
        <div>
          ...
          <div className="mt-4 space-y-4">
            <EquipmentCard 
              title={selectedModule?.nome || 'Módulo Padrão'} 
              icon={<Package />} 
              data={moduleData} 
            />
            {selectedInverters.map((inv, index) => (
              <EquipmentCard 
                ...
                data={inv.details ? {
                  ...
                  'Tipo de Rede': inv.details.tipo_rede || 'Não especificado',
                  'Nº de MPPTs': inv.details.numero_mppt?.toString() || 'N/A',
